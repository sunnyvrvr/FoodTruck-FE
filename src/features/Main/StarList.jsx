...
export default function StarList({setCurrentLocation, setCurrentAdress, setMyLocation, map}) {
    ...
    return (
        <div className='w-80 flex justify-between my-5 ml-5'>
            ...
            <div onClick={()=>handleStar('company')} className='w-20 h-7 border-2 border-background rounded-lg flex justify-center items-center bg-white font-bold'>
              <span><FaRegBuilding/></span>
              <span className='ml-1'>직장</span>
            </div>
            <div onClick={()=>handleStar('etc')} className='w-20 h-7 border-2 border-background rounded-lg flex justify-center items-center bg-white font-bold'>
              <span><FaRegStar/></span>
